...
def init_db():
    ...
    if os.getenv('STREAMLIT_SERVER_ADDRESS'):  # Running on Streamlit Cloud
        ...
        print(f"Database path on cloud: {db_path}")
            ...
            if original_url:  # Check if URL is provided
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if c.fetchone():
                            st.error("❌ This ID already exists. Please try another one.")
                            break
                        else:
                            # Insert new link
        ...
        if all_links:
            ...
            if link_id:
                ...
                if link:
                    ...
                    col1.metric("📊 Total Clicks", filtered_clicks)
                    ...
                    # (keeping it brief for brevity, but include all your existing chart code here)
...
with tab3:
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("""
            SELECT c.timestamp, COALESCE(l.custom_id, l.id), c.country, c.city, c.device_type, c.browser
            FROM clicks c
...
# TOOL 5: Click Tracker (hidden redirect) - FIXED with error handling
if 'id' in st.query_params:
    ...
    try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "database is locked" in str(e) and attempt < max_retries - 1:
                        ...
                        print(f"Database error in redirect: {e}")
        ...
        st.error(f"An error occurred: {str(e)}")
        # Still try to redirect if we have the URL
